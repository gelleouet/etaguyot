...
class HQL {
	 ...
	 * 
	 * @param session
	 * @param maxPagination <=0 pour désactiver la pagination et tout charger en 
	 * 	une seule fois
	 * @param closure
	 */
	void visit(Session session, int maxPagination, Closure closure) {
		int maxPage = maxPagination <= 0 ? Integer.MAX_VALUE : maxPagination

		// calcule le total pour créer les pages
		Query queryCount = createQuery(session, this.buildCount())
		QueryUtils.bindParameters(queryCount, params)
		Number totalCount = (Number) queryCount.uniqueResult()

		Query query = createQuery(session, this.build())
		QueryUtils.bindParameters(query, params)

		for (int offset=0; offset <= totalCount / maxPage; offset++) {
	 ...
	 * @return
	 */
	HQL parse(String query) throws AppException {
		int index
		...
		if (index == -1) {
			throw new AppException("Le mot clé FROM est introuvable !")
		}
